...
contract Governor {
    ...
    string public constant name = "FundDao Governor";
    ...
    /// @notice The address of the FundDao governance token
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(token.getPriorVotes(msg.sender, block.number - 1) >= PROPOSAL_THRESHOLD, "Governor::propose: proposer votes below proposal threshold");
